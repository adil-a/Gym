...
Implement the [Workplace Assistant](/latest/build-environments/real-world-environment) resources server (the verifier) — dynamic routing across 27 tools, per-session toolkit initialization, and state-based verification. The scale and routing patterns change here; the rest of the workflow (task data, agent/model config, rollout collection, training) matches the [single-step tutorial](/latest/build-environments/single-step-environment).
