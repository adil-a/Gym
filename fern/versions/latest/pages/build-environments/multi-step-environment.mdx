...
Build a multi-step tool-calling environment in about 30 minutes — the agent chains lookups across two tools and submits a final list, scored against ground truth with 1.0 for exact match and 0.0 otherwise. The tools and verification grow in complexity here; the rest of the workflow (task data, agent/model config, rollout collection, training) matches the [single-step tutorial](/latest/build-environments/single-step-environment).
...
The `simple_agent` (the agent harness) loop from the [single-step tutorial](/latest/build-environments/single-step-environment) drives this: the model emits a `function_call`, the tool executes, the result appends back into the conversation, and the model is called again with the updated context. The cycle repeats until the model produces a final text response or hits `max_steps`. The multi-step environment is where the agent learns to chain tool calls correctly.
