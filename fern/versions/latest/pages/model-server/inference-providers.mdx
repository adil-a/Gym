...
This table lists the server's own config fields. The `policy_*` keys you set in `env.yaml` are NeMo Gym's shared credential variables, reused across every model server; each provider config maps them onto these fields — `policy_base_url` → `base_url`, `policy_api_key` → `api_key`, and `policy_model_name` → `model`.
