...
| `responses_create_params` | Dict | Overrides for the `responses_create_params`, such as `temperature` and `max_output_tokens`. See [Generation parameters](#generation-parameters). |
| `resume_from_cache` | bool | Resume an interrupted run by skipping rows already completed. Default: `False`. See [Resume interrupted runs](#resume-interrupted-runs). |
...
- **Fallback.** If either the materialized inputs or the output file is missing, resume is skipped gracefully and the run starts fresh. With the default `resume_from_cache=False`, existing output is cleared before the run.
