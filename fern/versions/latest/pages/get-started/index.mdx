...
NeMo Gym is **a Gymnasium for LLMs** — RL training environments with a familiar `reset()` / `step()` API. This section gets you from zero to your first scored rollout, then to your first custom environment. Pick the path that matches your goal: skim the Quickstart for an immediate result, or work through Prerequisites → Installation → Configuration → Quickstart → Create Your First Environment for a thorough first run.
