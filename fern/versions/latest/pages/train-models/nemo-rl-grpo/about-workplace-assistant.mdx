...
Workplace Assistant (the training environment, the resources server) is a multi-step agentic tool-use benchmark that drops the model into a simulated office and asks it to complete realistic business tasks. Each rollout gets up to 6 tool-calling steps and is scored by comparing the final state of all 5 simulated databases against a ground-truth trajectory.

<NavButton href="/latest/train-models/nemo-rl-grpo" label="Back to Tutorial Overview" direction="back" />

## Prerequisites

- Read the [tutorial overview](#training-nemo-rl-grpo-index) for the training goals.

---

## How the Model Completes Tasks

Each task arrives as a natural-language user message. The model has to:

1. Parse intent.
2. Pick a tool sequence.
3. Infer parameters (for example, look up an email address or find matching customer records).
4. Execute every step needed to satisfy the request.

The hard cap is **6 tool calls per task**.

---

## Available Databases and Tools

Every task is a natural-language request executed against a shared tool set. Each **task instance** runs in isolated database instances so actions from different rollouts cannot interfere.

- **Databases**: Email, Calendar, Analytics, Project Management, Customer Relationship Manager (CRM).
- **Tools**: 27 functions distributed across the 5 databases.
- **Tasks**: Common business activities — sending email, scheduling meetings, updating CRM records, managing projects.

The full corpus lives in the [Workplace Assistant HuggingFace dataset](https://huggingface.co/datasets/nvidia/Nemotron-RL-agent-workplace_assistant).
...
The environment is a FastAPI resources server that executes tools and computes the reward. It uses **state-matching verification**: instead of forcing the model to reproduce a specific tool sequence, it compares the final database state to the ground truth.
...
With the environment, tools, and reward in mind, wire NeMo Gym into the NeMo RL training config:
