...
<NavButton href="/v0.2.1/get-started" label="Previous: Quickstart" direction="prev" />
...
Refer to [Configuration](/v0.2.1/reference/configuration) for additional `env.yaml` options.
...
- **Self-hosted models**: Use vLLM to serve your own models (see the [model-server-vllm](/v0.2.1/model-server/vllm))
...
<NavButton href="/v0.2.1/get-started/rollout-collection" label="Continue to Rollout Collection" direction="next" />

<Note>
Rollout collection is required before proceeding to tutorials like [Index](/v0.2.1/environment-tutorials) or training workflows. Complete it next.
