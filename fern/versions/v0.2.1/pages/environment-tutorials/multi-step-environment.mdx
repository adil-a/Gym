...
<Tip>
The `json.loads(output.arguments)` call is wrapped in `try/except` to handle cases where the model produces malformed JSON. Always guard against unparseable model output in your verify function. For more verification patterns, see [task-verification](/v0.2.1/about/concepts/task-verification).
...
[Next: Stateful Environment >](/v0.2.1/environment-tutorials/stateful-environment)
