...
<NavButton href="/main/get-started" label="Previous: Quickstart" direction="prev" />
...
Refer to [Configuration](/main/reference/configuration) for additional `env.yaml` options.
...
- **Self-hosted models**: Use vLLM to serve your own models (see the [model-server-vllm](/main/model-server/vllm))
...
<NavButton href="/main/get-started/rollout-collection" label="Continue to Rollout Collection" direction="next" />

<Note>
Rollout collection is required before proceeding to tutorials like [Index](/main/environment-tutorials) or training workflows. Complete it next.
