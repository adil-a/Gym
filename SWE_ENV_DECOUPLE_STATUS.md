...
- [in progress] CI on the PR (copyright-check + secrets-detector already pass; Lint + Test running).

## Current status
**Implemented, tested, and proven end-to-end with a real small model. PR open; watching CI.**
...
- apptainer/opensandbox not available locally → real-container testing limited to a docker/local provider; full infra run deferred to an apptainer/opensandbox box (command will be documented).

## Next steps
1. Read real sandbox API + mini_swe_agent_2 reference adapter.
2. Scaffold `swe_env` library + verifier; implement swe-bench-ext path.
3. Unit tests (FakeSandbox) green.
4. vLLM Qwen up; real verify-path test on a gold patch via docker/local provider.
5. PR + CI.
