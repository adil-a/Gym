...
"""Unit tests for the swe_agents app: config models, dataset processors, the OpenHands agent
runner, and the SWEBenchWrapper, including sample masking and the verifier POST contract."""
...
class TestRunOpenHandsAgent:
    ...
    @pytest.mark.asyncio
    async def test_run_decoupled_agent_timeout_sets_agent_timed_out(self, monkeypatch) -> None:
        """env.execute does not raise on agent timeout (it returns error_type="timeout"), so
        provision_and_collect succeeds and the except never fires. The worker must still set
        agent_timed_out from the surfaced error_type so the sample is masked."""
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            agent.config.metrics_fpath.parent.mkdir(parents=True, exist_ok=True)
            agent.config.metrics_fpath.write_text("{}")

            # provision returns a non-raising timeout: patch present but the agent self-driver
            # was killed by the timeout.
            async def _fake_provision(*_args, **_kwargs):
                return {"patch": "", "agent_error": None, "error_type": "timeout"}

            import responses_api_agents.swe_agents.swe_env_adapter as adapter

            monkeypatch.setattr(adapter, "provision_and_collect", _fake_provision)

            result = await agent._run_decoupled_agent()

            assert result is None  # decoupled contract: no report file
            persisted = swe_app.SWEBenchMetrics.model_validate_json(agent.config.metrics_fpath.read_text())
            assert persisted.agent_timed_out is True

    @pytest.mark.asyncio
    async def test_run_decoupled_agent_clean_run_not_timed_out(self, monkeypatch) -> None:
        """A clean agent run (no error_type) must not set agent_timed_out, so a valid sample is
        graded normally."""
        with tempfile.TemporaryDirectory() as tmpdir:
            agent = self._make_agent(
                tmpdir,
            ...
            async def _fake_provision(*_args, **_kwargs):
                return {"patch": "DIFF\n", "agent_error": None, "error_type": None}

            import responses_api_agents.swe_agents.swe_env_adapter as adapter

            monkeypatch.setattr(adapter, "provision_and_collect", _fake_provision)

            await agent._run_decoupled_agent()

            persisted = swe_app.SWEBenchMetrics.model_validate_json(agent.config.metrics_fpath.read_text())
            assert persisted.agent_timed_out in (False, None)
            assert persisted.patch_exists is True
...
class TestSWEBenchWrapperSetupParams:
    ...
    def test_basic_setup_params(self, monkeypatch) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # _setup_params does not build launch/eval commands; the verifier path owns launch +
            # eval, so these fields stay None.
...
class TestDecoupledCutover:
    """run() with eval_via_verifier: sample masking behavior and the verifier POST contract."""
    ...
    async def test_verify_patch_via_server_builds_request_and_parses_subset(self, monkeypatch) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # the worker persists the patch into metrics before run() POSTs to the verifier
        ...
        # image resolved from the docker formatter (id munged). The instance ships no test_command,
        # so we fall back to the conda+pytest default carrying the F2P+P2P node ids.
        ...
        # no per-row framework on this instance -> forwarded as empty (verifier auto-detects)
    ...
    async def test_verify_patch_via_server_forwards_instance_test_command_and_framework(self, monkeypatch) -> None:
        """A swe-bench-ext row carrying its own per-framework test_command/test_framework is
        forwarded verbatim (not clobbered by a hardcoded conda+pytest default), so multi-framework
        (cargo/go/npm/...) rows grade correctly."""
        wrapper = _create_wrapper(monkeypatch)
        monkeypatch.setattr(swe_app, "raise_for_status", AsyncMock(return_value=None))
        monkeypatch.setattr(
            swe_app,
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            await wrapper._verify_patch_via_server(params)

        md = wrapper.server_client.post.call_args.kwargs["json"]["responses_create_params"]["metadata"]
        # forwarded verbatim; the conda+pytest default must not clobber the row's own command
        assert md["test_command"] == "cargo test --offline"
        assert md["test_framework"] == "cargo"
    ...
    async def test_verify_patch_via_server_infra_error_is_masked_not_raised(self, monkeypatch) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            params.metrics_fpath.write_text(json.dumps({"model_patch": "<<DIFF>>"}))
            subset = await wrapper._verify_patch_via_server(params)
        # never raises; returns a masked subset so the agent still emits a present row
