...
"""Provider-neutral SWE environment library (issue #1249).
...
``nemo_gym.sandbox`` (PR #1377). Any agent imports this to provision and drive
its own working container; the (separate) ``resources_servers/swe_env`` verifier
imports the harness recipes + grading to score a patch in a fresh sandbox.

See plan: decouple SWE environment infrastructure from agent harnesses.
"""
